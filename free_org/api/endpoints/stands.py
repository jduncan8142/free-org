...
async def update_window(
    stand_id: int, window_id: int, updated_window: Window, session: Session = Depends(get_session)
):
    """
    ...
    if not db_window:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Window with ID {window_id} not found"
        )
