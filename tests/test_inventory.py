...
def test_get_all_inventory_with_filters(client, test_data):
    ...
    response = client.get("/api/inventory/?item_type=food")  # Use string value instead of enum
    assert response.status_code == status.HTTP_200_OK
    items = response.json()
    assert all(item["item_type"] == "food" for item in items)
...
def test_update_inventory_item_not_found(client):
    ...
    updated_data = {"name": "Updated Item Name", "description": "Updated Description", "quantity": 75}
...
def test_update_inventory_item_change_stand(client, session, test_data):
    ...
    another_stand = ConcessionStand(name="Another Stand", location="Another Location")
    session.add(another_stand)
    session.commit()

    # Update the inventory item to the new stand
    item_id = test_data["inventory_item"].id
    updated_data = {"name": "Updated Item Name", "stand_id": another_stand.id}
    response = client.put(f"/api/inventory/{item_id}", json=updated_data)
    assert response.status_code == status.HTTP_200_OK
    item = response.json()
...
def test_update_inventory_cross_threshold(client, session, test_data):
    ...
    updated_data = {"quantity": item.minimum_threshold - 1}
